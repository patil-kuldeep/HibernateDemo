...
public class PrincipalDaoImpl extends BaseDao<Principal> implements PrincipalDao {
    ...
    public int getExperience(int id) {
        Query query = getCurrentSession().createQuery("select yearsOfExperience from Principal where id: pId");
    ...
    public String getSchoolName(int id) {
        Query query = getCurrentSession().createQuery("from Principal where id: pId");
        query.setParameter("pId", id);
